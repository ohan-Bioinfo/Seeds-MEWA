...
import {
  MapPin, Wheat, Users, Calendar, Phone, Mail, Building2,
  TrendingUp, Package, Database, Send, FileCheck, Clock,
  CheckCircle2, AlertCircle, Sprout, Map, List, FlaskConical,
...
function KpiCard({ label: labelKey, target, icon: Icon, suffix = "", delay }: {
  label: string; target: number; icon: React.ElementType; suffix?: string; delay: number;
}) {
...
const FEATURED_SEEDS = [
  { id: "W 129", name: "قمح الجبل", species: "Triticum aestivum L", region: "Al-Baha", traits: ["Drought tolerant", "High protein"], availability: "Available", quantity: "500g" },
  { id: "C 25", name: "بن خولاني", species: "Coffea arabica", region: "Jazan", traits: ["Disease resistant", "High yield"], availability: "Available", quantity: "200g" },
  { id: "W 131", name: "قمح (صيداء)", species: "Triticum aestivum L", region: "Taif", traits: ["Heat tolerant", "Early maturity"], availability: "Limited", quantity: "250g" },
  { id: "C 27", name: "بن خولاني", species: "Coffea arabica", region: "Aseer", traits: ["Aromatic", "Premium quality"], availability: "Available", quantity: "300g" },
  { id: "W 222", name: "قمح دمياط", species: "Triticum aestivum L", region: "Riyadh", traits: ["Salt tolerant", "High biomass"], availability: "Available", quantity: "400g" },
  { id: "C 30", name: "بن عذبي", species: "Coffea arabica", region: "Al-Baha", traits: ["Shade adapted", "Consistent yield"], availability: "Available", quantity: "250g" },
];

const REQUEST_STEPS = [
  { step: 1, titleKey: "st.step1.title", descKey: "st.step1.desc", icon: Send, status: "active" as const },
  { step: 2, titleKey: "st.step2.title", descKey: "st.step2.desc", icon: FileCheck, status: "pending" as const },
  { step: 3, titleKey: "st.step3.title", descKey: "st.step3.desc", icon: FileCheck, status: "pending" as const },
  { step: 4, titleKey: "st.step4.title", descKey: "st.step4.desc", icon: Package, status: "pending" as const },
  { step: 5, titleKey: "st.step5.title", descKey: "st.step5.desc", icon: CheckCircle2, status: "pending" as const },
];
...
export default function Centers() {
  ...
  const filteredCenters = seedCenters.filter((center) => {
    ...
    if (selectedCrop && !center.crops.some((crop) => crop.name.toLowerCase().includes(selectedCrop.toLowerCase()))) return false;
    return true;
  });
  ...
  const totalAccessions = seedCenters.reduce((sum, c) => sum + c.tracking.accessionsStored, 0);
  const totalDistributed = Math.round(seedCenters.reduce((sum, c) => sum + c.seedExchange.distributed, 0) / 1000);
    ...
    <PageLayout>
      ...
      <section className="container -mt-8 relative z-10 mb-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <KpiCard label="st.kpi.stations" target={seedCenters.length} icon={Building2} delay={0} />
          <KpiCard label="st.kpi.staff" target={totalStaff} icon={Users} delay={0.08} />
          <KpiCard label="st.kpi.accessions" target={totalAccessions} icon={Database} delay={0.16} />
          <KpiCard label="st.kpi.distributed" target={totalDistributed} icon={Package} suffix="K" delay={0.24} />
        </div>
      </section>
        ...
        <Tabs defaultValue="map" className="w-full">
          <FadeUp>
            <TabsList className="flex flex-wrap w-full h-auto gap-1 mb-8 bg-secondary/50 p-1.5 rounded-xl">
              {[
                ...
                { value: "process", icon: FlaskConical, labelKey: "st.tab.process" },
                { value: "guidelines", icon: BookOpen, labelKey: "st.tab.guidelines" },
              ].map((tab) => {
              ...
              <CardContent className="pt-6">
                <div className={`flex flex-col md:flex-row gap-4 ${isRTL ? "md:flex-row-reverse" : ""}`}>
                  <div className="flex-1">
                    <label className="text-sm font-medium mb-2 block">{t("st.filter.region")}</label>
                    <select
                  ...
                  <div className="flex-1">
                    <label className="text-sm font-medium mb-2 block">{t("st.filter.crop")}</label>
                    <select
                      value={selectedCrop}
                  ...
                  {(selectedRegion || selectedCrop) && (
                    <div className="flex items-end">
                      <Button variant="outline" onClick={() => { setSelectedRegion(""); setSelectedCrop(""); }}>
                        {t("common.reset")}
                      </Button>
                    </div>
                  )}
                ...
                <FadeUp key={center.id} delay={idx * 0.05}>
                  ...
                  >
                    <CardHeader>
                      ...
                      <div className={`flex items-center gap-1 text-sm text-muted-foreground mt-1 ${isRTL ? "flex-row-reverse" : ""}`}>
                    ...
                    <CardContent className="space-y-3">
                      <div className="flex flex-wrap gap-1">
                        ...
                        {center.crops.length > 3 && (
                          <Badge variant="outline" className="text-xs">+{center.crops.length - 3}</Badge>
                        )}
                      </div>
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div className={`flex items-center gap-1 ${isRTL ? "flex-row-reverse" : ""}`}>
                          <Users className="h-3 w-3 text-muted-foreground" />
                          <span>{center.staff} {t("st.list.staff")}</span>
                        </div>
                        <div className={`flex items-center gap-1 ${isRTL ? "flex-row-reverse" : ""}`}>
                      ...
                      <div className="pt-2 border-t border-border">
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t("st.list.accessions")}</span>
          ...
          <TabsContent value="seeds" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2">
              {[
                { icon: Send, titleKey: "st.seeds.requestTitle", descKey: "st.seeds.requestDesc", btnKey: "st.seeds.newRequest", primary: true },
                { icon: Package, titleKey: "st.seeds.browseTitle", descKey: "st.seeds.browseDesc", btnKey: "st.seeds.viewCatalog", primary: false },
                { icon: FileCheck, titleKey: "st.seeds.trackTitle", descKey: "st.seeds.trackDesc", btnKey: "st.seeds.myRequests", primary: false },
              ].map((action, i) => {
                const AIcon = action.icon;
                return (
                  <FadeUp key={i} delay={i * 0.08}>
                    <Card className={`border-primary/20 hover:border-primary transition-colors ${action.primary ? "cursor-pointer" : ""}`}>
                      <CardHeader>
                      ...
                      <CardContent>
                        <Button className={`w-full ${action.primary ? "mewa-gradient text-white" : ""}`} variant={action.primary ? "default" : "outline"}>
                          {t(action.btnKey)}
                        </Button>
                      </CardContent>
                    </Card>
                        ...
                        <CardHeader className="pb-3">
                          <div className={`flex items-start justify-between ${isRTL ? "flex-row-reverse" : ""}`}>
                            ...
                            <Badge variant={seed.availability === "Available" ? "default" : "secondary"} className="text-xs">
                              {seed.availability === "Available" ? t("st.seeds.available") : t("st.seeds.limited")}
                            </Badge>
                          </div>
                        </CardHeader>
                        <CardContent className="space-y-3">
                          <div className="text-xs space-y-1">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("st.seeds.accession")}:</span>
                              <span className="font-mono font-medium">{seed.id}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("st.filter.region")}:</span>
                              <span className="font-medium">{seed.region}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("st.seeds.qty")}:</span>
            ...
            <FadeUp>
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>{t("st.process.workflowDesc")}</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-6">
                    {REQUEST_STEPS.map((step) => {
                      ...
                      return (
                        <div key={step.step} className={`flex gap-4 ${isRTL ? "flex-row-reverse" : ""}`}>
                          <div className="flex flex-col items-center">
                            ...
                            {step.step < 5 && <div className="w-0.5 h-12 bg-border my-2" />}
                          </div>
                          <div className="flex-1 pb-8">
                            <div className={`flex items-center gap-2 mb-1 ${isRTL ? "flex-row-reverse" : ""}`}>
                              <h4 className="font-semibold text-foreground">
                                {t("st.process.step")} {step.step}: {t(step.titleKey)}
                              </h4>
                              {step.status === "active" && <Badge className="bg-primary">{t("st.process.current")}</Badge>}
                            </div>
            ...
            <FadeUp delay={0.15}>
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>{t("st.process.statusDesc")}</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    {SAMPLE_REQUESTS.map((request) => (
                      <div key={request.id} className={`flex items-center justify-between p-4 border border-border rounded-lg hover:bg-secondary/50 transition-colors ${isRTL ? "flex-row-reverse" : ""}`}>
                        ...
                        <div className={`flex items-center gap-4 ${isRTL ? "flex-row-reverse" : ""}`}>
                          ...
                          <Button size="sm" variant="outline">{t("st.process.viewDetails")}</Button>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <FadeUp>
                <Card>
                  ...
                  <CardContent className="space-y-3 text-sm">
                    {["st.guide.elig1", "st.guide.elig2", "st.guide.elig3", "st.guide.elig4"].map((key) => (
                      <div key={key} className={`flex gap-3 ${isRTL ? "flex-row-reverse" : ""}`}>
              ...
              <FadeUp delay={0.1}>
                <Card>
                  ...
                  <CardContent className="space-y-3 text-sm">
                    {["st.guide.doc1", "st.guide.doc2", "st.guide.doc3", "st.guide.doc4"].map((key) => (
                      <div key={key} className={`flex gap-3 ${isRTL ? "flex-row-reverse" : ""}`}>
              ...
              <FadeUp delay={0.2}>
                <Card>
                  ...
                  <CardContent className="space-y-2 text-sm text-muted-foreground">
                    ...
                    {["st.guide.term1", "st.guide.term2", "st.guide.term3", "st.guide.term4", "st.guide.term5"].map((key) => (
                      <div key={key} className={`flex gap-2 ${isRTL ? "flex-row-reverse" : ""}`}>
              ...
              <FadeUp delay={0.3}>
                <Card>
                  ...
                  <CardContent className="space-y-3 text-sm">
                    <div>
                      <strong className="text-foreground">{t("st.guide.coordinator")}</strong>
                      <p className="text-muted-foreground">Dr. Ahmed Al-Qahtani</p>
                    </div>
                    <div>
                      <strong className="text-foreground">{t("st.guide.email")}</strong>
                      <p className="text-muted-foreground">seedexchange@mewa.gov.sa</p>
                    </div>
                    <div>
                      <strong className="text-foreground">{t("st.guide.phone")}</strong>
                      <p className="text-muted-foreground">+966 11 XXX XXXX ext. 234</p>
                    </div>
                    <div>
                      <strong className="text-foreground">{t("st.guide.hours")}</strong>
                      <p className="text-muted-foreground">{t("st.guide.hoursVal")}</p>
                    </div>
                  </CardContent>
                </Card>
              </FadeUp>
              ...
              <div className="space-y-6 mt-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    ...
                    { val: selectedCenter.crops.length, labelKey: "st.dialog.crops" },
                    { val: selectedCenter.established, labelKey: "st.dialog.est" },
                  ].map((item) => (
                    <div key={item.labelKey} className="text-center p-3 bg-secondary rounded-lg">
                ...
                {/* Crops */}
                <div>
                  <h3 className={`font-semibold text-lg mb-3 flex items-center gap-2 ${isRTL ? "flex-row-reverse" : ""}`}>
                    <Wheat className="h-5 w-5 text-primary" />
                    {t("st.dialog.cropsPlanted")}
                  </h3>
                  <div className="space-y-2">
                    {selectedCenter.crops.map((crop, idx) => (
                      <div key={idx} className={`flex justify-between items-center p-3 bg-secondary rounded-lg ${isRTL ? "flex-row-reverse" : ""}`}>
                        <div>
                        ...
                        <div className={isRTL ? "text-left" : "text-right"}>
                          ...
                          <div className="text-xs text-muted-foreground">{crop.varieties} {t("st.dialog.varieties")}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="space-y-2">
                    {selectedCenter.activities.map((activity, idx) => (
                      <div key={idx} className="p-3 bg-secondary rounded-lg">
                        ...
                        <p className="text-sm">{language === "ar" ? activity.descriptionAr : activity.description}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 bg-secondary rounded-lg">
                      <div className="text-2xl font-bold text-primary">{selectedCenter.seedExchange.distributed.toLocaleString()}</div>
                      <div className="text-xs text-muted-foreground">{t("st.dialog.distYear")}</div>
                    </div>
                    <div className="p-3 bg-secondary rounded-lg">
                      <div className="text-2xl font-bold text-primary">{selectedCenter.seedExchange.received.toLocaleString()}</div>
                      <div className="text-xs text-muted-foreground">{t("st.dialog.recYear")}</div>
                    </div>
                  </div>
                  <div className="mt-3">
                    <div className="text-sm font-medium mb-2">{t("st.dialog.partners")}:</div>
                    <div className="flex flex-wrap gap-1">
                ...
                {/* Contact */}
                <div>
                  <h3 className="font-semibold text-lg mb-3">{t("st.dialog.contact")}</h3>
                  <div className="space-y-2">
                    <div className={`flex items-center gap-2 text-sm ${isRTL ? "flex-row-reverse" : ""}`}>
                      <Phone className="h-4 w-4 text-primary" />
                      <a href={`tel:${selectedCenter.contact.phone}`} className="hover:underline">{selectedCenter.contact.phone}</a>
                    </div>
                    <div className={`flex items-center gap-2 text-sm ${isRTL ? "flex-row-reverse" : ""}`}>
                      <Mail className="h-4 w-4 text-primary" />
                      <a href={`mailto:${selectedCenter.contact.email}`} className="hover:underline">{selectedCenter.contact.email}</a>
                    </div>
                  </div>
                </div>
              </div>
